...
global $db;
	...
	if($mode=='edit') {
		...
		echo $html->select_measuring_units($name, $objType->measuring_units, ($asset->getId()) ? $asset->$name : $asset->assetType->$name);
		//($asset->$name != "")? $asset->$name : $defaut
	...
	elseif($mode=='new'){
		...
		echo $html->select_measuring_units($name, $objType->measuring_units, ($asset->getId()) ? $defaut : $asset->assetType->$name);
		//($asset->$name != "")? $asset->$name : $defaut
		...
		echo measuring_units_string($asset->$name, $objType->measuring_units);

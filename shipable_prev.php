 ...
 */
require('config.php');
//TODO passer l'extrafield date de livraison en conf
...
$sql .= " WHERE c.fk_statut NOT IN (" . Commande::STATUS_CANCELED . "," . Commande::STATUS_CLOSED . ") AND p.fk_product_type=0 AND p.rowid IS NOT NULL";
...
foreach($TLinesToDisplay as $lineid) {
    if(!empty($TLines[$lineid])) {
        ...
        else if(empty($TLines[$lineid]->array_options['options_svpm_date_livraison'])) $color = '#dedb8d';
        else $color = '#de8d8d';

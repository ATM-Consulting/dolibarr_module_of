...
// Setup page goes here
	...
	$var=!$var;
	print '<tr '.$bc[$var].'>';
	print '<td>'.$langs->trans("set_ASSET_COEF_WS").'</td>';
	print '<td align="center" width="20">&nbsp;</td>';
	print '<td align="right" width="300" style="white-space:nowrap;">';
	print '<form method="POST" action="'.$_SERVER['PHP_SELF'].'">';
	print '<input type="hidden" name="token" value="'.$_SESSION['newtoken'].'">';
	print '<input type="hidden" name="action" value="set_ASSET_COEF_WS">';
	print $formCore->texte('', 'ASSET_COEF_WS', (empty($conf->global->ASSET_COEF_WS) ? '' : $conf->global->ASSET_COEF_WS), 5,255);
	print '<input type="submit" class="button" value="'.$langs->trans("Modify").'">';
	print '</form>';
	print '</td></tr>';
...
function showParameters(&$formCore) {
	...
	?><form action="<?php echo $_SERVER['PHP_SELF'] ?>" name="load-<?php echo $typeDoc ?>" method="POST" enctype="multipart/form-data">
		...
		<table width="100%" class="noborder">
			...
			<?php $var=!$var; ?>
			<tr id ="USE_DEFAULT_WAREHOUSE" <?php echo $bc[$var]; ?>>
			...
			<?php $var=!$var; ?>
			<tr <?php echo $bc[$var]; ?> id="WAREHOUSE_TO_MAKE" class="pair" <?php if (empty($conf->global->ASSET_USE_DEFAULT_WAREHOUSE)) echo "style='display:none;'" ?>>
			...
			<?php $var=!$var; ?>
			<tr <?php echo $bc[$var]; ?> id="WAREHOUSE_NEEDED" <?php if (empty($conf->global->ASSET_USE_DEFAULT_WAREHOUSE)) echo "style='display:none;'" ?>>

...
$input = $formCore->number("", "OF_NB_TICKET_PER_PAGE",getDolGlobalString('OF_NB_TICKET_PER_PAGE')?$conf->global->OF_NB_TICKET_PER_PAGE:0,10,1,-1);
...
$input = $form::selectarray('DEFAULT_ETIQUETTES', $liste, getDolGlobalString('DEFAULT_ETIQUETTES')?$conf->global->DEFAULT_ETIQUETTES:0);
setup_print_input_form_part('DEFAULT_ETIQUETTES', $langs->trans('CHOOSE_CUSTOM_LABEL'), '', array(), $input, $tooltip);

$input = $formCore->texte('', 'ABRICOT_WKHTMLTOPDF_CMD', (!getDolGlobalString('ABRICOT_WKHTMLTOPDF_CMD') ? '' : $conf->global->ABRICOT_WKHTMLTOPDF_CMD), 80,255,' placeholder="wkhtmltopdf" ');
setup_print_input_form_part('ABRICOT_WKHTMLTOPDF_CMD', false, 'ABRICOT_WKHTMLTOPDF_CMD_DESC', array(), $input);


print '<tbody class="default-etiquette-sub-conf" data-target="2" style="display: '.(getDolGlobalString('DEFAULT_ETIQUETTES') && ($conf->global->DEFAULT_ETIQUETTES != 2) ?'none':'').'" >';
...
if(!empty($conf->workstationatm->enabled)){
	$input = $form->multiselectarray('OF_WORKSTATION_NON_COMPLIANT', TWorkstation::getWorstations($PDOdb), getDolGlobalString('OF_WORKSTATION_NON_COMPLIANT') ? explode(',',$conf->global->OF_WORKSTATION_NON_COMPLIANT) : '',0, 0, '', 0, 300);
...
setup_print_title('ParamLinkedToOFOthers');
	...
	print $formCore->texte('', 'OF_COEF_MINI_TU_1', getDolGlobalString('OF_COEF_MINI_TU_1')?$conf->global->OF_COEF_MINI_TU_1:'', 10, 10);
...
print $formCore->texte('', 'OF_COEF_WS', (!getDolGlobalString('OF_COEF_WS') ? '' : $conf->global->OF_COEF_WS), 5,255);
print '<input type="submit" class="button" value="'.$langs->trans("Modify").'">';
print '</form>';
print '</td></tr>';
...
print $form::selectarray('OF_DELIVERABILITY_REPORT_SUPPLIERORDER_DATE_EXTRAFIELD', $liste, getDolGlobalString('OF_DELIVERABILITY_REPORT_SUPPLIERORDER_DATE_EXTRAFIELD')?$conf->global->OF_DELIVERABILITY_REPORT_SUPPLIERORDER_DATE_EXTRAFIELD:'',1);
print '<input type="submit" class="button" value="'.$langs->trans("Modify").'">';
print '</form>';
print '</td></tr>';
...
print $form::selectarray('OF_DELIVERABILITY_REPORT_ORDER_DATE_EXTRAFIELD', $liste, getDolGlobalString('OF_DELIVERABILITY_REPORT_ORDER_DATE_EXTRAFIELD')?$conf->global->OF_DELIVERABILITY_REPORT_ORDER_DATE_EXTRAFIELD:'',1);
print '<input type="submit" class="button" value="'.$langs->trans("Modify").'">';
print '</form>';
print '</td></tr>';
...
print $form::selectarray('OF_DELIVERABILITY_REPORT_PROPAL_DATE_EXTRAFIELD', $liste, getDolGlobalString('OF_DELIVERABILITY_REPORT_PROPAL_DATE_EXTRAFIELD')?$conf->global->OF_DELIVERABILITY_REPORT_PROPAL_DATE_EXTRAFIELD:'',1);
print '<input type="submit" class="button" value="'.$langs->trans("Modify").'">';
print '</form>';
print '</td></tr>';
...
function showParameters(&$form) {
	...
	?><form action="<?php echo $_SERVER['PHP_SELF'] ?>" name="load-of" method="POST" enctype="multipart/form-data">
		...
		<table width="100%" class="noborder">
			...
			<tr class="pair" id="WAREHOUSE_TO_MAKE" class="pair" <?php if (!getDolGlobalString('ASSET_USE_DEFAULT_WAREHOUSE')) echo "style='display:none;'" ?>>
			...
			<tr class="impair" id="WAREHOUSE_NEEDED" <?php if (!getDolGlobalString('ASSET_USE_DEFAULT_WAREHOUSE')) echo "style='display:none;'" ?>>

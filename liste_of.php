...
function _liste(&$PDOdb)
{
	...
	if($mode =='supplier_order') {
		...
		, ofe.status, ofe.fk_user,ofe.total_estimated_cost, (SELECT SUM(qty_needed * pmp) as prix  FROM ".MAIN_DB_PREFIX."assetOf_line WHERE fk_assetOf = ofe.rowid AND type != 'TO_MAKE') AS total_cost, '' AS printTicket ";
